...
ENV \
	...
	SETUP_DEPENDENCIES_UTILITIES="bash unzip jq htop git mercurial gnupg man man-pages fcgi openssh-client nfs-utils iproute2" \
...
ADD imports/bin/setup /usr/local/bin/setup/1526689919
ADD imports/bin/config /usr/local/bin/config/1526689919
...
ADD imports/confd/toml/exports.toml /etc/confd/conf.d/exports.toml
ADD imports/confd/tmpl/exports.tmpl /etc/confd/templates/exports.tmpl


RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1526689919 1>/dev/stdout 2>/dev/stderr
