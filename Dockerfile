...
ENV \
	...
	SETUP_DEPENDENCIES_UTILITIES="bash unzip jq htop git mercurial gnupg man man-pages fcgi py-pip ansible py-psutil" \
...
ADD imports/bin/setup /usr/local/bin/setup/1525872745
ADD imports/bin/config /usr/local/bin/config/1525872745


RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1525872745 1>/dev/stdout 2>/dev/stderr
