...
ENV \
	...
	SETUP_DEPENDENCIES_UTILITIES="bash unzip jq htop git mercurial gnupg man man-pages fcgi openssh-client ansible nfs-utils iproute2 file" \
...
ADD imports/bin/setup /usr/local/bin/setup/1526939228
ADD imports/bin/config /usr/local/bin/config/1526939228
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1526939228 1>/dev/stdout 2>/dev/stderr
