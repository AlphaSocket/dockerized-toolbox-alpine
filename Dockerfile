...
ENV \
	...
	SETUP_DEPENDENCIES_UTILITIES="bash unzip jq htop git gnupg man man-pages fcgi py-pip" \
	...
	SETUP_DEPENDENCIES_PYTHON="python python-dev python3 py-pip" \
	SETUP_DEPENDENCIES_DB="mysql-client redis" \
	SETUP_DEPENDENCIES_GOOGLE="py-crcmod libc6-compat openssh-client" \
	SETUP_DEPENDENCIES_RUNTIME="bash unzip jq htop git gnupg man man-pages fcgi py-pip libc6-compat php7 php7-curl php7-dom php7-gd php7-ctype php7-gettext php7-iconv php7-json php7-mbstring php7-mcrypt php7-mysqli php7-opcache php7-openssl php7-pdo php7-pdo_dblib php7-pdo_mysql php7-pdo_pgsql php7-pdo_sqlite php7-pear php7-pgsql php7-phar php7-posix php7-session php7-soap php7-sockets php7-sqlite3 php7-xml php7-zip php7-zlib nodejs nodejs-npm python python-dev python3 py-pip mysql-client redis py-crcmod libc6-compat openssh-client" \
...
ADD imports/bin/setup /usr/local/bin/setup/1525134768
ADD imports/bin/config /usr/local/bin/config/1525134768


RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1525134768 1>/dev/stdout 2>/dev/stderr
